...
//   const CLOSED_STATUSES = ["Rejected", "Accepted", "Withdrawn"];
...
//     if (CLOSED_STATUSES.includes(selectApplicant.status)) {
...
//     // Update UI state for statuses requiring additional input
//     // This allows the form fields to show
//     if (newStatus === "Rejected" || newStatus === "Interview") {
//       setCurrentStatus(newStatus);
//       return;
//     }

//     // For other statuses, proceed with immediate API update
//     await updateApplicationStatus(newStatus);
//   };

//   // Separate function to handle the actual API update
//   const updateApplicationStatus = async (newStatus, additionalData = {}) => {
//     if (!selectApplicant?._id) return;
...
//     const success = await updateApplicationStatus("Rejected", {
...
//   // Handle interview scheduling confirmation
...
//     const success = await updateApplicationStatus("Interview", {
...
//  try{
//   // extract userId and file from the path
//   const partPaths = resumePath.split("/");
//   const userId = partPaths[partPaths.length -2];
//   const filename = partPaths[partPaths - 1];
//   console.log("Download Request", {userId, filename, fullPath:resumePath});

//   const response = await axiosInstance.get(API_PATHS.DOWNLOAD_RESUME(userId, filename), {});
...
//       if (status === 404) {
//         toast.error("Resume file not found. The file may have been deleted or moved.");
//       } else if (status === 403) {
//         toast.error("Access denied - you don't have permission to download this file");
//       } else if (status === 500) {
//         toast.error("Server error - please contact support");
//       } else {
//         toast.error(`Download failed (Error ${status})`);
//       }
//     } else if (error.request) {
...
//   const isApplicationClosed = CLOSED_STATUSES.includes(selectApplicant.status);
...
//             //onClick={() => handleResumeDownload(applicant?.resume)}
//             onClick={handleDownloadResume(handleResumeDownload(applicant?.resumePath))}
...
//               disabled={isUpdatingStatus || isApplicationClosed}
...
//             {currentStatus === "Rejected" && !isApplicationClosed && (
//               <div className="mt-3">
//                 <label
//                   className="block mb-2 text-sm text-gray-700 font-medium"
...
//             {/* Show interview date field when "Interview" is selected */}
//             {currentStatus === "Interview" && !isApplicationClosed && (
//               <div className="mt-3">
//                 <label
//                   className="block mb-2 text-sm text-gray-700 font-medium"
...
//                   className="w-full mt-2 px-4 py-2 text-white font-medium rounded-lg hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed bg-blue-600 transition-colors"
//                 >
...
const handleResumeDownload = async () => {
  if (!selectApplicant?.resumeDownloadUrl) {
    toast.error("Resume not available");
    return;
  }
  ...
  try {
    const response = await axiosInstance.get(
      selectApplicant.resumeDownloadUrl,
      { responseType: "blob" }
    );
    ...
    const filename =
      disposition?.match(/filename="(.+)"/)?.[1] ?? "resume.pdf";
      ...
      >
        ...
        <div className="p-6 space-y-6">
          ...
          <button
            ...
            onClick={handleDownloadResume(handleResumeDownload())}
            disabled={!isDownloading || !applicant.resume}
            aria-label="Download applicant resume"
          >
            <Download className="w-4 h-4" />
